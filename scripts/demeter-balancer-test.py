...
USDs = project.dependencies["OpenZeppelin"]["4.9.3"].IERC20.at(USDs_Address)
...
@dataclass
class RewardTokenData:
    token: bytes
    tknManager: bytes

def createFarm(farmDeployer, creatorAddress):
    """
    Creates a farm using the provided farm deployer and creator address.

    Args:
        farmDeployer: The farm deployer contract instance.
        creatorAddress: The address of the creator.

    Returns:
        The transaction object for creating the farm.
    """
    ...
    rewardTknData = RewardTokenData(USDs.address, creator.address)
    data = FarmData(
        OWNER.address,
...
def main():
    """
    The main function that deploys the farm deployer and creates a farm.

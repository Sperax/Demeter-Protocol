...
contract Demeter_CamelotFarm is Ownable, ReentrancyGuard, Initializable {
    ...
    function deposit(uint256 _tokenId, bool _lockup) external nonReentrant {
        ...
        // Transfer the nft position to the farm
        INFTPool(nftPool).safeTransferFrom(account, address(this), _tokenId);

        // @dev Add the deposit to the user's deposit list
        deposits[account].push(userDeposit);

        emit Deposited(account, _lockup, _tokenId, liquidity);
    }

...
contract Demeter_E20_farm is Ownable, ReentrancyGuard, Initializable {
    ...
    // _tokenId - unique id for deposit
    ...
    function deposit(uint256 _amount, bool _lockup) external nonReentrant {
        ...
        _subscribeRewardFund(COMMON_FUND_ID, userDeposit.tokenId, _amount);

        if (_lockup) {
            // Add lockup fund subscription to the user's deposit
            userDeposit.cooldownPeriod = cooldownPeriod;
            _subscribeRewardFund(LOCKUP_FUND_ID, userDeposit.tokenId, _amount);
        }
        ...
        deposits[account].push(userDeposit);

        emit Deposited(account, _lockup, userDeposit.tokenId, _amount);
    }
    ...
    function recoverERC20(address _token) external onlyOwner nonReentrant {
        ...
        require(farmToken != _token, "Can't withdraw farm token");

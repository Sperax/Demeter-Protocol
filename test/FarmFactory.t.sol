...
contract UpdateFeeParamsTest is FarmFactoryTest {
    ...
    function test_RevertWhen_InvalidAmount() public useKnownActor(FACTORY_OWNER) initialized deployerRegistered {
        vm.expectRevert(abi.encodeWithSelector(FarmFactory.InvalidAmount.selector));
        FarmFactory(factory).updateFeeParams(owner, USDS, 0, EXTENSION_FEE_PER_DAY);
        vm.expectRevert(abi.encodeWithSelector(FarmFactory.InvalidAmount.selector));
        FarmFactory(factory).updateFeeParams(owner, USDS, FEE_AMOUNT, 0);
    }
